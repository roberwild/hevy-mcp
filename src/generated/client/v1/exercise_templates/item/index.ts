// @ts-ignore
import type {
	BaseRequestBuilder,
	Parsable,
	ParsableFactory,
	RequestConfiguration,
	RequestInformation,
	RequestsMetadata,
} from "@microsoft/kiota-abstractions";
/* tslint:disable */
/* eslint-disable */
// Generated by Microsoft Kiota
// @ts-ignore
import {
	type ExerciseTemplate,
	createExerciseTemplateFromDiscriminatorValue,
} from "../../../models/index.js";

/**
 * Builds and executes requests for operations under /v1/exercise_templates/{exerciseTemplateId}
 */
export interface WithExerciseTemplateItemRequestBuilder
	extends BaseRequestBuilder<WithExerciseTemplateItemRequestBuilder> {
	/**
	 * Get a single exercise template by id.
	 * @param requestConfiguration Configuration for the request such as headers, query parameters, and middleware options.
	 * @returns {Promise<ExerciseTemplate>}
	 */
	get(
		requestConfiguration?: RequestConfiguration<object> | undefined,
	): Promise<ExerciseTemplate | undefined>;
	/**
	 * Get a single exercise template by id.
	 * @param requestConfiguration Configuration for the request such as headers, query parameters, and middleware options.
	 * @returns {RequestInformation}
	 */
	toGetRequestInformation(
		requestConfiguration?: RequestConfiguration<object> | undefined,
	): RequestInformation;
}
/**
 * Uri template for the request builder.
 */
export const WithExerciseTemplateItemRequestBuilderUriTemplate =
	"{+baseurl}/v1/exercise_templates/{exerciseTemplateId}";
/**
 * Metadata for all the requests in the request builder.
 */
export const WithExerciseTemplateItemRequestBuilderRequestsMetadata: RequestsMetadata =
	{
		get: {
			uriTemplate: WithExerciseTemplateItemRequestBuilderUriTemplate,
			responseBodyContentType: "application/json",
			adapterMethodName: "send",
			responseBodyFactory: createExerciseTemplateFromDiscriminatorValue,
		},
	};
/* tslint:enable */
/* eslint-enable */
